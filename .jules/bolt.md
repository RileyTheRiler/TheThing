...
## 2024-05-25 - Dense Grid Lookups
**Learning:** Using Dictionaries for coordinate lookups `(x, y)` in a small, fixed-size dense grid (20x20) incurs significant hashing overhead (~35%) compared to flat array indexing.
**Action:** For fixed-size grids, prefer flat arrays mapped by `y * width + x` over dictionaries to reduce overhead in hot paths like A*.
